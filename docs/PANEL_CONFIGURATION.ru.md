...
> [!TIP]
> **Иконки из шрифтов (MaterialDesign Icons и др.)** можно вставлять прямо в текстовые поля (`text`, `options` и т.п.) в форматах, описанных в документации openHASP:
> - Для `json` / `jsonl` payload используйте `\\uXXXX` (ровно 4 hex-цифры) — например: `"text":"\\uE64A Hello!"`.
> - Если кодпоинт больше `0xFFFF`, используйте UTF-16 surrogate pair, например: `"text":"\\uDB81\\uDC25 Hello!"`.
>
> Подробно: [Fonts → Icons / Encoding](https://www.openhasp.com/0.7.0/design/fonts/#icons).
>
> Вся документация по страницам и объектам интерфейса:
> - [Pages](https://www.openhasp.com/0.7.0/design/pages/)
> - [Objects](https://www.openhasp.com/0.7.0/design/objects/)
...
## 9.1. Отправка MQTT-команд через MCP (OpenHasp)

Через MCP-операцию `mqtt_publish` можно отправлять команды на панель так же, как это описано в документации openHASP:

- Вариант 1 (топик `.../command`, payload — строка команды): например `page next` или `p1b2.val=25`.
- Вариант 2 (топик `.../command/<keyword>`, payload — значение): например `command/p1b2.val` с payload `25`.
- Вариант 3 (advanced): указать `relative_topic`, но он **обязан** начинаться с `command` (без выхода за пределы корня панели).

См. официально: [MQTT Commands](https://www.openhasp.com/0.7.0/commands/mqtt/).

## 9.2. MCP-операции для проверки результата агентом

Чтобы агент мог не только применять конфиг, но и проверять итог на панели, в OpenHasp MCP доступны отдельные операции:

- `set_page` — переключить активную страницу панели (`panel_id`, `page`);
- `get_page` — получить текущую активную страницу из состояния устройства в БД;
- `idle` — отправить команду `idle` (`off`, `short`, `long`) или запросить текущее состояние `idle` пустым payload;
- `request_screenshot` — отправить команду скриншота по MQTT и получить кандидаты HTTP URL скриншота по IP панели.
- `analyze_panel_config` — анализировать `panel_config`: проверить структуру страниц/объектов, шаблоны и (опционально) существование `%Object.property%` в `osysHome`;
- `compile_page_jsonl` — сгенерировать список `jsonl` строк, которые плагин отправил бы для конкретной страницы (`page_index`), с опциональным разрешением биндингов текущими значениями.
- `patch_panel_config` — безопасно обновить корневые ключи `panel_config` без полного ручного overwrite:
  - `set` — добавить/заменить ключи;
  - `unset` — удалить ключи;
  - `if_match` — опциональная проверка ревизии;
  - `reload` — сразу отправить обновлённый конфиг на панель.

Примечания:

- Для пробуждения экрана используйте `idle` со `state=off`.
- Скриншот по URL работает, если на панели включен HTTP-сервер.
